...
contract MaguePresale is Ownable {
    IERC20 public token;
    ...
    constructor(
        ...
        tokenMetadataUsdt = IERC20Metadata(_usdtAddress);
        paymentAddress = _paymentAddress;
        priceFeed = AggregatorV3Interface(_priceFeed);

        // stage data
        stages[1] = Stage(1, 1500000000000000, 1732384800, 1734976800); //stage 1
        stages[2] = Stage(2, 200000000000000, 1734976801, 1737655200); //stage 2
        stages[3] = Stage(3, 300000000000000, 1737655201, 1740333600); //stage 3
        stages[4] = Stage(4, 400000000000000, 1740333601, 1742752800); //stage 4
        currentStageId = 4;
    }
    ...
    // buyTokenWithUsdt funtion to buy tokens
    // @param _amount No of tokens to buy
    function buyTokenWithUsdt(uint256 _amount) public payable {
        ...
        require(
            tokenUsdt.allowance(msg.sender, address(this)) >= _totalPayUsd,
            "Not enough allowance!"
        );

        //payment USDT transfer
        require(
            tokenUsdt.transferFrom(msg.sender, paymentAddress, _totalPayUsd),
    ...
    function setTokenUsdt(address _tokenUsdt) public onlyOwner {
        require(_tokenUsdt != address(0), "Token is zero address!");
        tokenUsdt = IERC20(_tokenUsdt);
        tokenMetadataUsdt = IERC20Metadata(_tokenUsdt);
    }
    ...
    function addStage(
        ...
        currentStageId += 1;
        stages[_id] = Stage(_id, _price, _start, _end);
    }
